...
dotnet nuget push "${files[0]}" --api-key "$NugetApiKey" --source "$NugetSource" --no-service-endpoint
echo "Pushing packed package to GitHub Packages..."
dotnet nuget push "${files[0]}" --api-key "$PackagesApiKey" --source "$PackagesSource" --no-service-endpoint
